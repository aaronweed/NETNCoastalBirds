...
`r PlotWeeklyBands(survey = "incubation", species = "DCCO", current_yr = params$year)`
...
`r PlotWeeklyBands(survey = "incubation", species = "GBBG", current_yr = params$year)`
...
`r PlotWeeklyBands(survey = "incubation", species = "HERG", current_yr = params$year)`
...
#### Review surveys by date for (`r params$year`)

`r SumCreche_Daily`

```{r,cache=F,echo=F, warning =FALSE, include=T,fig.pos='h',results='asis', fig.height= 6, fig.width= 12,message=FALSE}
options(knitr.kable.NA = '')
...
# plot current year counts per life stage compared to historical data across All Outer Islands


PlotHistBands(survey = "creche", current_yr = params$year)


# Output table of raw counts per lifestage

df<- filter(creche, year== params$year & !Island == "All Islands" & !variable == "Average creche size") %>% select(Island,Date = time,variable,value) %>% spread(variable,value) 

kable(df, caption = paste0("Table 12. Counts per life stage of Common Eider during Primary Surveys ", params$year))  %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
