...
plt_var<- function(type = "boxplot", survey, species, var) {
  ...
  if(type == "boxplot") {
    ...
    y2<- ggplot(data= df, aes(x= Island, y= value, fill= Island, group= Island, shape = as.factor(year))) +
    geom_boxplot(alpha= 0.3, notch= F) +geom_jitter(aes( color= color), size= 3)
  
  }
    
    if(type == "violin"){
    
    y2<- ggplot(data= df, aes(x= Island, y= value, fill= Island, group= Island, shape = as.factor(year))) + geom_violin(alpha= 0.3)+
 ...
 y2 <- (y2 + 
  ...
  theme_bw()+
  guides(fill = "none", color= "none") + # This removes the legend for fill
  labs(y = "Count", x ="", shape = "Year")+
  ggtitle(paste0("Variation in counts of ", species," ", var, " from 2007 to ", params$year, "."))+
...
**Staff should evaluate the tables to ensure that at least one incubation survey was completed per year and that at least one survey was designated as the Primary Survey.**
...
kableExtra::kbl(DCCO[,c("Island","time","value")] %>% spread(time, value), caption= "Table 4. Annual counts of Double-crested Cormorant",padding =2) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
