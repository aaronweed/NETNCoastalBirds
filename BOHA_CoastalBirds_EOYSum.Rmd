...
ggplot(data= filter(DCCO, stat == "max"), aes(x= Island, y= value, fill= Island, group= Island)) +
  geom_boxplot(alpha= 0.3, notch= F, outliers = TRUE)+ 
...
# create box plot
ggplot(data= filter(GBBG, stat == "max"), aes(x= Island, y= value, fill= Island, group= Island)) +
  geom_boxplot(alpha= 0.3, notch= F, outliers = TRUE)+ 
  geom_jitter(aes(shape = shape, color= color), size= 3) +
  scale_color_manual(values = c("black", "red"))+ 
  scale_shape_manual(values = c(19, 8))+ 
  ...
  theme(legend.position = "none")+
  labs(y = "Incubating Adults")+
  ggtitle(paste0("Variation in counts of incubating GBBG adults from 2007 to ", params$year, " by Island. The red asterisks denotes current year count."))+
  theme(axis.text.y = element_text(color="black", vjust= 0.5, size = 16)) +
         theme(axis.text.x = element_text(angle = 0,  vjust=0,size = 12 )) +
          theme(axis.text.y = element_text(size = 12 )) +
         theme(strip.text.x= element_text(size=12, face=c("bold.italic"))) +
...
# create box plot
ggplot(data= filter(HERG, stat == "max"), aes(x= Island, y= value, fill= Island, group= Island)) +
  geom_boxplot(alpha= 0.3, notch= F, outliers = TRUE)+ 
  geom_jitter(aes(shape = shape, color= color), size= 3) +
  scale_color_manual(values = c("black", "red"))+ 
  scale_shape_manual(values = c(19, 8))+ 
  ...
  theme(legend.position = "none")+
  labs(y = "Incubating Adults")+
  ggtitle(paste0("Variation in counts of incubating HERG adults from 2007 to ", params$year, " by Island. The red asterisks denotes current year count."))+
  theme(axis.text.y = element_text(color="black", vjust= 0.5, size = 16)) +
         theme(axis.text.x = element_text(angle = 0,  vjust=0,size = 12 )) +
          theme(axis.text.y = element_text(size = 12 )) +
         theme(strip.text.x= element_text(size=12, face=c("bold.italic"))) +
...
ground_notes<- nests_raw %>% filter(year %in% params$year ) %>% group_by(Species_Code, Island, Segment, Date) %>% 
  mutate(c_Obs_Notes = paste0(unique(Obs_Notes), collapse = ";")) %>% distinct() %>% select(Species_Code, Island, Segment, Survey_Complete, Date, c_Obs_Notes)  %>% arrange(Date, Island, Segment)

ground_notes<- ground_notes[!duplicated(ground_notes$c_Obs_Notes), ] %>% as_tibble()
...
kable(ground_methods %>% filter(Species_Code == "COEI"), caption = "Table 14. Nest counts per status category.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create box plot of all life stages (Species_Unit) for all islands

ggplot(data= filter(nests, Species_Code == "COEI", variable == "Nests"), aes(x= Island, y= value, fill= Island, group= Island)) +
  geom_boxplot(alpha= 0.3, notch= F, outliers = TRUE)+ 
  geom_jitter(aes(shape = shape, color= color), size= 3) +
  scale_color_manual(values = c("black", "red"))+ 
  #coord_flip()+
  ...
  theme(legend.position = "none")+
  labs(y = "Count", x ="")+
  ggtitle(paste0("Variation in ground-based nest counts per life stage from 2007 to ", params$year, ". The red asterisks denotes current year count."))+
  theme(axis.text.y = element_text(color="black", vjust= 0.5, size = 16)) +
         theme(axis.text.x = element_text(angle = 0,  vjust=0,size = 12 )) +
          theme(axis.text.y = element_text(size = 12 )) +
         theme(strip.text.x= element_text(size=12, face=c("bold.italic"))) +
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "DCCO"), caption = "Table 16. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "GBBG"), caption = "Table 18. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "HERG"), caption = "Table 20. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "BCNH"), caption = "Table 22. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "GLIB"), caption = "Table 24. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "GREG"), caption = "Table 26. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
...
# create table showing current year counts by nest status
kable(ground_methods %>% filter(Species_Code == "SNEG"), caption = "Table 28. Nest counts per status category.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T)
