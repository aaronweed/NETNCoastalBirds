...
#' @param data A \code{data.frame}  of coastal bird observations summarized for plotting.
#'  Typically from \code{\link{SumIncubationByDate}}, \code{\link{SumIncubationByYear}}, 
#'  \code{\link{CrecheSum}}, \code{\link{NestByDate}}, or \code{\link{NestsByYear}}
#' @param island A vector of island names (e.g., "Calf"). To view summaries across all 
#' islands, "All Islands"
#' @param species  A  vector of species name codes, e.g. "BCNH"
#' @param var Select a variable to plot, typically a life stage (e.g., Eggs, Nests, 
#' Creche size). Defaults to all values.
#' @param scale Convert to log scale by entering "log"
#' @param year Calendar year(s) to view data by. Useful when wanting to view seasonal 
...
PlotBirds <- function(data,
  ...
  if (!anyNA(species))
    graphdata <- graphdata[graphdata$Species_Code %in% species, ]
  
  if (!anyNA(island))
    graphdata <- graphdata[graphdata$Island %in% island, ]
  
  if (!anyNA(var))
    graphdata <- graphdata[graphdata$variable %in% var, ]
  
  if (!anyNA(year))
    graphdata <-
    graphdata[graphdata$year %in% year, ] # for subsetting data ByDate
  
  if (facet == "Island")
    graphdata <- graphdata[!graphdata$Island %in% "All Islands", ]
  ...
  if (scale == "log") {
    y2 <-
      ...
      ggtitle(paste0(if (!anyNA(var))
        var, " counts of ", graphdata$CommonName[1], " per ", facet))
  }
