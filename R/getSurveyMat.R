...
#' @param survey A character vector. Accepts "nest", "creche", "incubation" or "AMOY". 
...
#' @param island A  vector of island names. To view summariaes across all islands, 
#' "All Islands"
...
getSurveyMat <- function(survey, df= NULL, island=NA, year= NA, species=NA) {
  
  if(is.null(df)){
  if(survey == "nest") 
    df <- as.data.frame(NestsByDate(x)[1])
  
  if(survey == "incubation") 
    df <- as.data.frame(as.data.frame(incub_DCCO_Gull_sumfunc(x)[1]))
