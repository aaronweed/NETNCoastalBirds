...
#' @param facet Plot the data into separate facets by Island, Species, or variable. Defaults to Island. If \code{overlay = "island"} set \code{facet} to "CommonName", "Species_Code" , "FullLatinName" or "variable".
...
#### TOGGLE plot title (above plot) 
  ...
  ###DEFINE THEME
  y2 <- (y2 +
         {if (is.numeric(graphdata$time)) scale_x_continuous(breaks= seq(min(graphdata$time),max(graphdata$time),1))}+
