...
#### Import dataframes from DB to access survey effort by event per species ----
  ...
  ####### create new vectors to match for joining tables######## ----
  Seg_Size$pk_SegmentID <- Seg_Size$fk_SegmentID
  species_tlu$Target_Spp<-species_tlu$TargetSpp_Group
  ...
  seg_event<- 
    left_join(Isl_Seg, Seg_Size, by= "pk_SegmentID") %>% # joins tables to create table of survey effort per segement
    select(Island, Segment, Survey_Class, Survey_Type, Target_Spp, Survey_Size, Size_Units, Active) %>% 
    left_join(event,., by=c("Survey_Class", "Survey_Type" , "Island","Segment")) %>% # add on segment survey data
    left_join(species_tlu,., by="Target_Spp") %>%   #Add species codes to survey effort to remove target species grouping for later joining
